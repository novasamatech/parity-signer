...
internal fun OnboardingScreen1() {
	Column(
		Modifier.fillMaxSize(1f).background(
			brush = Brush.linearGradient(
				start = Offset(Float.POSITIVE_INFINITY,0.0f),
				end = Offset(0.0f, Float.POSITIVE_INFINITY),
				colors = listOf(MaterialTheme.colors.pink500, Color(0xFF1E1E23)),
			)
		)
	) {
		PageIndicatorLine(
		...
		Spacer(modifier = Modifier.padding(top = 30.dp).weight(0.2f))
		Image(
			painter = painterResource(id = R.drawable.onboarding_2),
			contentDescription = null,
			modifier = Modifier.padding(horizontal = 24.dp).fillMaxWidth(1f).weight(0.7f)
		)
