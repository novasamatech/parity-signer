...
use definitions::{error::TransferContent, error_signer::{ErrorSigner, GeneralVerifierForContent, InputSigner, Signer}, history::Event, keyring::{NetworkSpecsKey, VerifierKey}, network_specs::{ValidCurrentVerifier, Verifier}, qr_transfers::ContentAddSpecs};
        ...
        None => {
            match checked_info.verifier {
                Verifier(None) => {
                    ...
                    Ok(Action::Stub(format!("\"warning\":[{}],\"new_specs\":[{}]", warning_card, specs_card), checksum, StubNav::AddSpecs(network_specs_key)))
                },
                Verifier(Some(ref new_verifier_value)) => {
                    ...
                    match general_verifier {
                        Verifier(None) => {
                            ...
                            let specs_card = Card::NewSpecs(&specs).card(&mut index,0);
                            let checksum = stub.store_and_get_checksum(database_name)?;
                            Ok(Action::Stub(format!("\"verifier\":[{}],\"warning\":[{}],\"new_specs\":[{}]", verifier_card, warning_card, specs_card), checksum, StubNav::AddSpecs(network_specs_key)))
                        },
                        _ => {
                            if checked_info.verifier == general_verifier {
                                ...
                                let specs_card = Card::NewSpecs(&specs).card(&mut index, 0);
                                let checksum = stub.store_and_get_checksum(database_name)?;
                                Ok(Action::Stub(format!("\"verifier\":[{}],\"new_specs\":[{}]", verifier_card, specs_card), checksum, StubNav::AddSpecs(network_specs_key)))
                            }
                            else {
        ...
        Some(ValidCurrentVerifier::Custom(custom_verifier)) => {
            match custom_verifier {
                Verifier(None) => {
                    match checked_info.verifier {
                        Verifier(None) => {
                            ...
                            else {Err(ErrorSigner::Input(InputSigner::SpecsKnown{name: specs.name, encryption: specs.encryption}))} 
                        },
                        Verifier(Some(ref new_verifier_value)) => {
                            let verifier_card = Card::Verifier(new_verifier_value).card(&mut index,0);
                ...
                Verifier(Some(ref old_verifier_value)) => {
                    match checked_info.verifier {
                        Verifier(None) => Err(ErrorSigner::Input(InputSigner::NeedVerifier{name: specs.name, verifier_value: old_verifier_value.to_owned()})),
                        Verifier(Some(ref new_verifier_value)) => {
                            let verifier_card = Card::Verifier(new_verifier_value).card(&mut index,0);
                            if checked_info.verifier == general_verifier {
                                let hold = Hold::get(&verifier_key, database_name)?;
                            ...
                            else {Err(ErrorSigner::Input(InputSigner::AddSpecsVerifierChanged{name: specs.name, old_verifier_value: old_verifier_value.to_owned(), new_verifier_value: new_verifier_value.to_owned()}))}
                        },
                    }
                },
            }
            ...
            match general_verifier {
                ...
                Verifier(Some(ref old_general_verifier_value)) => {
                    if checked_info.verifier == general_verifier {
                        ...
                        match checked_info.verifier {
                            Verifier(None) => Err(ErrorSigner::Input(InputSigner::NeedGeneralVerifier{content: GeneralVerifierForContent::Network{name: specs.name}, verifier_value: old_general_verifier_value.to_owned()})),
                            Verifier(Some(new_general_verifier_value)) => Err(ErrorSigner::Input(InputSigner::GeneralVerifierChanged{content: GeneralVerifierForContent::Network{name: specs.name}, old_general_verifier_value: old_general_verifier_value.to_owned(), new_general_verifier_value})),
                        }
                    }
                }
            }
