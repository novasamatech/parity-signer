...
pub (crate) fn parse_transaction (data_hex: &str, database_name: &str) -> Result<Action, ErrorSigner> {
...
pub (crate) fn decode_signable_from_history (found_signable: &SignDisplay, database_name: &str) -> Result<String, ErrorSigner> {
    ...
    match found_solution {
        Some(a) => Ok(a),
        None => Err(ErrorSigner::AllExtensionsParsingFailed{network_name, errors: error_collection})
    }
}
