...
fn collect_set (verifier_key: &VerifierKey, chainspecs: &Tree, metadata: &Tree) -> Result<(Vec<MetaValues>, Vec<NetworkSpecs>), ErrorSigner> {
...
impl Hold {
    ...
    pub (crate) fn upd_stub (&self, stub: TrDbColdStub, verifier_key: &VerifierKey, former_verifier: &Verifier, new_verifier: &ValidCurrentVerifier, hold_release: HoldRelease, database_name: &str) -> Result<TrDbColdStub, ErrorSigner> {
        ...
        for x in self.metadata_set.iter() {out = out.remove_metadata(x)}
        for x in self.network_specs_set.iter() {out = out.remove_network_specs(x, &ValidCurrentVerifier::Custom(former_verifier.to_owned()), &general_verifier)}
