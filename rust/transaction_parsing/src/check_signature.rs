use definitions::{
    error::{ErrorSigner, InputSigner, NotHexSigner, Signer, TransferContent},
    helpers::unhex,
...
) -> Result<InfoPassedCrypto, ErrorSigner> {
    ...
    match &data_hex[2..4] {
        "00" => {
            ...
            if ed25519::Pair::verify(&signature, &message, &pubkey) {
                ...
                Err(ErrorSigner::Input(InputSigner::BadSignature))
            }
        }
        "01" => {
        ...
        _ => Err(ErrorSigner::Input(InputSigner::EncryptionNotSupported(
