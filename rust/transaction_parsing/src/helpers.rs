...
pub (crate) fn specs_by_name (network_name: &str, encryption: &Encryption, database_name: &str) -> Result<NetworkSpecs, ErrorSigner> {
    ...
    match found_network_specs {
        Some(a) => Ok(a),
        None => Err(ErrorSigner::NotFound(NotFoundSigner::HistoryNetworkSpecs{name: network_name.to_string(), encryption: encryption.to_owned()})),
    }
}
...
pub fn accept_meta_values (meta_values: &MetaValues, database_name: &str) -> Result<bool, ErrorSigner> {
    ...
    match metadata.get(meta_key.key()) {
        Ok(Some(a)) => {
            ...
            else {Err(ErrorSigner::Input(InputSigner::SameNameVersionDifferentMeta{name: meta_values.name.to_string(), version: meta_values.version}))}
        },
        Ok(None) => Ok(true),
...
pub fn specs_are_new (new: &NetworkSpecsToSend, database_name: &str) -> Result<bool, ErrorSigner> {
    ...
    match chainspecs.get(network_specs_key.key()) {
        Ok(Some(encoded_known_network_specs)) => {
            ...
            if (old.base58prefix != new.base58prefix)|(old.decimals != new.decimals)|(old.encryption != new.encryption)|(old.name != new.name)|(old.unit != new.unit) {return Err(ErrorSigner::Input(InputSigner::ImportantSpecsChanged(network_specs_key)))}
