...
use crate::error::{
    AddressGeneration, AddressGenerationCommon, DatabaseSigner, EntryDecodingSigner, ErrorSigner,
    ExtraAddressGenerationSigner, GeneralVerifierForContent, InputSigner, InterfaceSigner,
    KeyDecodingSignerDb, KeyDecodingSignerInterface, MetadataError, MismatchSigner, NotFoundSigner,
    NotHexSigner, ParserDecodingError, ParserError, ParserMetadataError, TransferContent,
};
...
pub fn signer_errors() -> Vec<ErrorSigner> {
    ...
    error_set.push(ErrorSigner::Database(DatabaseSigner::EntryDecoding(
        EntryDecodingSigner::HistoryEntry(135),
    )));
    error_set.push(ErrorSigner::Database(DatabaseSigner::EntryDecoding(
