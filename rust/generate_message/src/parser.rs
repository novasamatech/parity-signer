                ...
                match arg.as_str() {
                    ...
                    "load_metadata"|"add_specs" => {
                        ...
                        let content = match content_key {
                            Some(x) => {
                                match x.as_str() {
                                    "-a" => {
                                        if name.is_some() {return Err(ErrorActive::CommandParser(CommandParser::Unexpected(CommandUnexpected::KeyAContent)))}
                                        Content::All
                                    },
                                    "-n" => {
                                        match name {
                    ...
                    "make" => {
                        ...
                        while let Some(x) = args.next() {
                            ...
                            match x.as_str() {
                                "-crypto" => {
                                    if crypto_type_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::CryptoKey)))}
                                ...
                                "-msgtype" => {
                                    if msg_type_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::MsgType)))}
                                ...
                                "-verifier" => {
                                    if verifier_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::Verifier)))}
                                ...
                                "-payload" => {
                                    if payload_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::Payload)))}
                                ...
                                "-signature" => {
                                    if signature_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::Signature)))}
                                ...
                                "-name" => {
                                    if name.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::Name)))}
                        ...
                        let crypto = match crypto_type_found {
                            Some(x) => {
                                match x {
                                    ...
                                    CryptoType::None => {
                                        if verifier_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::Unexpected(CommandUnexpected::VerifierNoCrypto)))}
                                        if signature_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::Unexpected(CommandUnexpected::SignatureNoCrypto)))}
                                        Crypto::None
                                    },
                                }
                            },
                    ...
                    "sign" => {
                        ...
                        while let Some(x) = args.next() {
                            ...
                            match x.as_str() {
                                "-sufficient" => {
                                    if sufficient_crypto_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::SufficientCrypto)))}
                    ...
                    "remove" => {
                        ...
                        while let Some(a) = args.next() {
                            match a.as_str() {
                                "-title" => {
                                    if info_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::Remove)))}
                    ...
                    "derivations" => {
                        ...
                        while let Some(a) = args.next() {
                            match a.as_str() {
                                "-title" => {
                                    if found_title.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::DerivationsTitle)))}
                                ...
                                "-payload" => {
                                    if found_payload.is_some() {return Err(ErrorActive::CommandParser(CommandParser::DoubleKey(CommandDoubleKey::Payload)))}
...
fn process_verifier_and_signature (verifier_found: Option<VerKey>, signature_found: Option<Entry>, encryption: Encryption) -> Result<Crypto, ErrorActive> {
    match verifier_found {
        ...
        Some(VerKey::Alice) => {
            if signature_found.is_some() {return Err(ErrorActive::CommandParser(CommandParser::Unexpected(CommandUnexpected::AliceSignature)))}
            Ok(Crypto::Alice(encryption))
        },
        None => Err(ErrorActive::CommandParser(CommandParser::NeedKey(CommandNeedKey::Verifier))),
    }
}
...
fn get_needed_signature(signature_found: Option<Entry>) -> Result<Vec<u8>, ErrorActive> {
    match signature_found {
        ...
        None => Err(ErrorActive::CommandParser(CommandParser::NeedKey(CommandNeedKey::Signature))),
    }
}
