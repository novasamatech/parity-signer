...
pub fn gen_add_network (instruction: Instruction) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match instruction.set {
        ...
        // `-f` key: do NOT run rps calls, produce ALL requested files from existing database
            match instruction.content {
                Content::All => {
                // creating add_network files for all existing database metadata entries
                    if metadata.len() == 0 {return Err(Box::from("No metadata entries in the database."))}
                    for x in metadata.iter() {
                        if let Ok(a) = x {
                            match add_f_from_metadata_entry(a, &address_book, &chainspecs) {
                                Ok(()) => (),
                ...
                Content::Name(names) => {
                // creating add_network files for a given set of names from existing database metadata entries
                    for name in names.iter() {
                        match add_f_from_name(name, &address_book, &chainspecs, &metadata) {
                            Ok(()) => (),
                            Err(e) => error_occured(e, instruction.pass_errors)?,
                        }
                    }
        ...
        // `-d` key: do NOT update the database, make rpc calls, and produce ALL requested output files
            let flag = false; // going to make `add_network` message
            
            match instruction.content {
                Content::All => {
                // creating add_network for all address book entries
                ...
                Content::Name(names) => {
                // creating add_network for a given set of names in address book
                    for name in names.iter() {
                ...
                // creating add_network for a given set of addresses
                    for address in addresses.iter() {
                        match sign_me_prep_add_network_from_address(&address, &chainspecs) {
                            Ok(()) => (),
                            Err(e) => error_occured(e, instruction.pass_errors)?,
                        }
                    }
