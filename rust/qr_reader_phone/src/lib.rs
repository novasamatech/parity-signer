...
pub fn get_payload (line: &str) -> anyhow::Result<Vec<u8>> {
...
pub fn get_length (line: &str) -> anyhow::Result<u32> {
...
//        println!("dealing with element of fountain qr");
        ...
        let number_of_messages = length/(new_packet.len() as u32);
...
pub fn decode_sequence (jsonline: &str) -> anyhow::Result<String> {
...
mod tests {
    ...
    fn number_of_frames_fountain() {
        ...
        let length = get_length(line).unwrap();
        assert!(length == 15, "Expected 15, decoded {}", length);
    }
