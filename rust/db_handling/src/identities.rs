...
#[cfg(feature = "active")]
use definitions::{error_active::{Active, ErrorActive, InputActive}, qr_transfers::ContentDerivations};
#[cfg(feature = "signer")]
use definitions::{error::{AddressGeneration, SpecsKeySource}, error_signer::{ErrorSigner, ExtraAddressGenerationSigner, InputSigner, InterfaceSigner, Signer}};
#[cfg(any(feature = "active", feature = "signer"))]
use definitions::{crypto::Encryption, error::{AddressGenerationCommon, ErrorSource}, helpers::multisigner_to_public, history::{Event, IdentityHistory}, keyring::{NetworkSpecsKey, AddressKey}, network_specs::NetworkSpecs, users::AddressDetails};

#[cfg(feature = "signer")]
use crate::{db_transactions::TrDbColdDerivations, interface_signer::addresses_set_seed_name_network, helpers::{get_network_specs, get_address_details}};
#[cfg(any(feature = "active", feature = "signer"))]
use crate::{db_transactions::TrDbCold, helpers::{make_batch_clear_tree, open_db, open_tree, upd_id_batch}, manage_history::events_to_batch, network_details::get_all_networks};
...
#[cfg(any(feature = "active", feature = "signer"))]
pub(crate) fn create_address<T: ErrorSource> (database_name: &str, input_batch_prep: &[(AddressKey, AddressDetails)], path: &str, network_specs: &NetworkSpecs, seed_name: &str, seed_phrase: &str) -> Result<(Vec<(AddressKey, AddressDetails)>, Vec<Event>), T::Error> {
