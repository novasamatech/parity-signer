...
use definitions::{
    ...
    error_signer::{
        ErrorSigner, ExtraAddressGenerationSigner, InputSigner, InterfaceSigner, NotFoundSigner,
        Signer,
    },
...
#[cfg(any(feature = "active", feature = "signer"))]
pub(crate) fn get_all_addresses<T: ErrorSource>(
    database_name: &str,
) -> Result<Vec<(MultiSigner, AddressDetails)>, T::Error> {
    ...
    for (address_key_vec, address_entry) in identities.iter().flatten() {
        ...
        let (multisigner, address_details) =
            AddressDetails::process_entry_with_key_checked::<T>(&address_key, address_entry)?;
...
#[cfg(any(feature = "active", feature = "signer"))]
pub fn get_addresses_by_seed_name<T: ErrorSource>(
    database_name: &str,
    seed_name: &str,
) -> Result<Vec<(MultiSigner, AddressDetails)>, T::Error> {
    Ok(get_all_addresses::<T>(database_name)?
        .into_iter()
...
#[cfg(any(feature = "active", feature = "signer"))]
fn has_parent_with_exposed_secret<T: ErrorSource>(
    new_cropped_path: &str,
    new_is_passworded: bool,
    seed_name: &str,
    database_name: &str,
    ...
    let mut out = false;
    for (_, address_details) in get_addresses_by_seed_name::<T>(database_name, seed_name)?.iter() {
...
pub(crate) fn create_address<T: ErrorSource>(
    ...
    match number_in_current {
        ...
        None => {
            ...
            // another network.
            let secret_exposed = has_parent_with_exposed_secret::<T>(
                cropped_path,
                has_pwd,
                seed_name,
                database_name,
            ...
            match identities.get(address_key.key()) {
                ...
                Ok(Some(address_entry)) => {
                    ...
                    if secret_exposed && !address_details.secret_exposed {
                        return Err(<T>::secret_exposed_mismatch(multisigner, address_details));
                    }
    ...
    match is_passworded(path) {
        ...
        Ok(false) => {
            ...
            for (multisigner, address_details) in
                get_all_addresses::<Signer>(database_name)?.into_iter()
            {
...
pub fn remove_seed(database_name: &str, seed_name: &str) -> Result<(), ErrorSigner> {
    ...
    let id_set = get_addresses_by_seed_name::<Signer>(database_name, seed_name)?;
...
pub fn export_secret_key(
    ...
    let database_addresses =
        get_addresses_by_seed_name::<Signer>(database_name, expected_seed_name)?;
    ...
    let mut qr = {
        if address_details.network_id.contains(network_specs_key) {
            ...
            match png_qr_from_string(
                ...
                Err(e) => {
                    secret.zeroize();
                    return Err(ErrorSigner::Qr(e.to_string()));
                }
            }
        } else {
