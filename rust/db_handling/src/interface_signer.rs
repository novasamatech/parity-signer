...
use definitions::{
    error::{
        AddressGenerationCommon, DatabaseSigner, ErrorSigner, ErrorSource, InterfaceSigner,
        NotFoundSigner, Signer,
    },
    helpers::{
        make_identicon_from_multisigner, multisigner_to_encryption, multisigner_to_public, pic_meta,
    },
    keyring::{print_multisigner_as_base58, AddressKey, NetworkSpecsKey, VerifierKey},
...
pub fn print_new_seed(seed_name: &str) -> Result<String, ErrorSigner> {
    ...
    let hex_identicon =
        match make_identicon_from_multisigner(&MultiSigner::Sr25519(sr25519_pair.public())) {
            Ok(a) => hex::encode(a),
            Err(_) => String::new(),
        };
    Ok(format!(
...
mod tests {
    ...
    fn print_ids_seed_name_network() {
        ...
        populate_cold(dbname, Verifier(None)).unwrap();
        let horrible_print = print_identities_for_seed_name_and_network(
            dbname,
            "Alice",
