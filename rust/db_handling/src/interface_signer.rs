...
use definitions::{error::{AddressKeySource, DatabaseSigner, ErrorSigner, ExtraAddressKeySourceSigner, InterfaceSigner, NotFoundSigner, Signer}, helpers::{multisigner_to_public}, keyring::{NetworkSpecsKey, AddressKey, print_multisigner_as_base58}, network_specs::NetworkSpecs, print::export_complex_vector, users::AddressDetails};
...
pub fn export_key (database_name: &str, address_key: &AddressKey, expected_seed_name: &str, network_specs_key: &NetworkSpecsKey) -> Result<String, ErrorSigner> {
    let network_specs = get_network_specs(database_name, &network_specs_key)?;
    let address_details = get_address_details(database_name, &address_key)?;
    if address_details.seed_name != expected_seed_name {return Err(ErrorSigner::Interface(InterfaceSigner::SeedNameNotMatching{address_key: address_key.to_owned(), expected_seed_name: expected_seed_name.to_string(), real_seed_name: address_details.seed_name}))}
    ...
    let qr_prep = {
        ...
        else {return Err(ErrorSigner::NotFound(NotFoundSigner::NetworkSpecsKeyForAddress{network_specs_key: network_specs_key.to_owned(), address_key: address_key.to_owned()}))}
    };
    Ok(format!("{{\"qr\":\"{}\",\"public\":\"{}\",\"base58\":\"{}\",\"identicon\":\"{}\",\"seed_name\":\"{}\",\"path\":\"{}\",\"network_name\":\"{}\",\"network_logo\":\"{}\"}}", hex::encode(qr_prep), hex::encode(public_key), address_base58, hex::encode(identicon), address_details.seed_name, address_details.path, network_specs.name, network_specs.logo))
}
