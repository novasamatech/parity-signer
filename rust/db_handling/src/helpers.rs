...
use constants::{
    ADDRTREE, DANGER, GENERALVERIFIER, METATREE, SETTREE, SPECSTREE, TYPES, VERIFIERS,
};
use definitions::{
    danger::DangerRecord,
    error::{
        DatabaseSigner, EntryDecodingSigner, ErrorSigner, ErrorSource, NotFoundSigner, Signer,
    },
    ...
    match verifiers.get(verifier_key.key()) {
        Ok(Some(verifier_encoded)) => match <CurrentVerifier>::decode(&mut &verifier_encoded[..]) {
            Ok(a) => match a {
                ...
                CurrentVerifier::Dead => {
                    return Err(ErrorSigner::DeadVerifier(verifier_key.to_owned()))
                }
            },
            Err(_) => {
...
pub fn get_valid_current_verifier(
    ...
    match try_get_valid_current_verifier(verifier_key, database_name)? {
        Some(a) => Ok(a),
        None => {
            return Err(ErrorSigner::NotFound(NotFoundSigner::CurrentVerifier(
...
pub fn get_network_specs(
    ...
    match try_get_network_specs(database_name, network_specs_key)? {
        Some(a) => Ok(a),
        None => {
            return Err(ErrorSigner::NotFound(NotFoundSigner::NetworkSpecs(
...
pub fn get_address_details(
    ...
    match try_get_address_details(database_name, address_key)? {
        Some(a) => Ok(a),
        None => {
            return Err(ErrorSigner::NotFound(NotFoundSigner::AddressDetails(
...
pub fn get_meta_values_by_name_version<T: ErrorSource>(
    ...
    match metadata.get(meta_key.key()) {
        ...
        Ok(None) => {
            return Err(<T>::metadata_not_found(
...
mod tests {
    ...
    use crate::{
        cold_default::{
            populate_cold_no_metadata, populate_cold_release, signer_init_no_cert,
            signer_init_with_cert,
        },
        manage_history::{device_was_online, reset_danger_status_to_safe},
    };
    ...
    fn display_general_verifier_properly() {
        ...
        assert!(
            print == r#""public_key":"","identicon":"","encryption":"none""#,
            "Got: {}",
            print
        );
    ...
    fn not_find_mock_verifier() {
        ...
        populate_cold_no_metadata(dbname, Verifier(None)).unwrap();
        let verifier_key = VerifierKey::from_parts(
            &hex::decode("62bacaaa3d9bb01313bb882c23615aae6509ab2ef1e7e807581ee0b74c77416b")
                .unwrap(),
        );
