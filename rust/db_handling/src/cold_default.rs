...
use constants::{
    ADDRTREE, DANGER, GENERALVERIFIER, HISTORY, METATREE, SETTREE, SPECSTREE, TRANSACTION, TYPES,
    VERIFIERS,
};
use defaults::{
    default_general_verifier, get_default_chainspecs, get_default_types_content,
    get_default_verifiers, get_nav_test_metadata, get_release_metadata, get_test_metadata,
};
use definitions::{
    danger::DangerRecord,
    error::{Active, ErrorActive, ErrorSigner, ErrorSource, Signer},
    history::Event,
...
fn reset_cold_database_no_addresses(
    ...
    TrDbCold::new()
        ...
        .set_transaction(make_batch_clear_tree::<Active>(
            &database_name,
            TRANSACTION,
        )?) // clear transaction
