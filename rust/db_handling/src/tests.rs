...
mod tests {
    ...
    use crate::{
        ...
        interface_signer::{
            addresses_set_seed_name_network, backup_prep, derive_prep, dynamic_path_check,
            export_key, first_network, guess, metadata_details, network_details_by_key,
            print_all_identities, print_all_seed_names_with_identicons,
            print_identities_for_seed_name_and_network, show_all_networks,
            show_all_networks_with_flag, show_types_status, SeedDraft,
        },
        manage_history::{
    ...
    fn print_seed_names() {
        ...
        let print = print_all_seed_names_with_identicons(dbname, &vec![String::from("Alice")])
            .unwrap()
    ...
    fn must_check_for_valid_derivation_phrase() {
        ...
        assert!(is_passworded("//path///password///password")
            .expect("password///password is a password"));
    ...
    fn creating_derivation_2() {
            ...
            derivation_check("Alice", "//Alice", &network_id_0, dbname).unwrap()
        {
            ...
            panic!("New derivation has no password, existing derivation has password and is diffenent.");
        }
        assert!(
            try_create_address("Alice", ALICE_SEED_PHRASE, "//Alice", &network_id_0, dbname)
                .is_ok(),
    ...
    fn creating_derivation_3() {
            ...
            derivation_check("Alice", "//Alice///secret", &network_id_0, dbname).unwrap()
        {
            ...
            panic!("New derivation has password, existing derivation has no password and is diffenent.");
        }
        assert!(
            try_create_address(
                "Alice",
