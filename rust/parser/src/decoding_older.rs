...
fn decode_primitive (found_ty: &str, data: &[u8], indent: u32, short_specs: &ShortSpecs) -> Option<DecodedOut> {
    match found_ty {
        "bool" => decode_known_length::<bool>(data, found_ty, indent).map_or_else(|_| None, Some),
        "u8" => decode_primitive_with_flags::<u8>(data, &mut None, false, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "u16" => decode_primitive_with_flags::<u16>(data, &mut None, false, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "u32" => decode_primitive_with_flags::<u32>(data, &mut None, false, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "u64" => decode_primitive_with_flags::<u64>(data, &mut None, false, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "u128" => decode_primitive_with_flags::<u128>(data, &mut None, false, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "Percent" => decode_perthing::<Percent>(data, false, found_ty, indent).map_or_else(|_| None, Some),
        "Perbill" => decode_perthing::<Perbill>(data, false, found_ty, indent).map_or_else(|_| None, Some),
        "PerU16" => decode_perthing::<PerU16>(data, false, found_ty, indent).map_or_else(|_| None, Some),
        "Compact<u8>" => decode_primitive_with_flags::<u8>(data, &mut None, true, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "Compact<u16>" => decode_primitive_with_flags::<u16>(data, &mut None, true, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "Compact<u32>" => decode_primitive_with_flags::<u32>(data, &mut None, true, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "Compact<u64>" => decode_primitive_with_flags::<u64>(data, &mut None, true, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "Compact<u128>" => decode_primitive_with_flags::<u128>(data, &mut None, true, false, found_ty, indent, short_specs).map_or_else(|_| None, Some),
        "Compact<Percent>" => decode_perthing::<Percent>(data, true, found_ty, indent).map_or_else(|_| None, Some),
        "Compact<Perbill>" => decode_perthing::<Perbill>(data, true, found_ty, indent).map_or_else(|_| None, Some),
        "Compact<PerU16>" => decode_perthing::<PerU16>(data, true, found_ty, indent).map_or_else(|_| None, Some),
        _ => None,
    }
}
...
fn decode_complex (found_ty: &str, mut data: Vec<u8>, meta: &OlderMeta, type_database: &[TypeEntry], indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
...
pub (crate) fn process_as_call (mut data: Vec<u8>, meta: &OlderMeta, type_database: &[TypeEntry], mut indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
...
fn deal_with_option (inner_ty: &str, mut data: Vec<u8>, type_database: &[TypeEntry], indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
    if inner_ty == "bool" {
...
///
/// The function outputs the DecodedOut value in case of success.
fn deal_with_vector (inner_ty: &str, mut data: Vec<u8>, type_database: &[TypeEntry], indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
    let pre_vector = get_compact::<u32>(&data)?;
    let mut fancy_output_prep: Vec<OutputCard> = Vec::new();
...
///
/// The function outputs the DecodedOut value in case of success.
fn deal_with_array (inner_ty: &str, number_of_elements: u32, mut data: Vec<u8>, type_database: &[TypeEntry], indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
    let mut fancy_output_prep: Vec<OutputCard> = Vec::new();
...
fn special_case_identity_fields (data: Vec<u8>, type_database: &[TypeEntry], indent: u32) -> Result<DecodedOut, ParserError> {
...
fn special_case_balance (found_ty: &str, data: Vec<u8>, indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
    match found_ty {
        "Balance" | "T::Balance" | "BalanceOf<T>" | "BalanceOf<T, I>" => {
            decode_primitive_with_flags::<u128>(&data, &mut None, false, true, "u128", indent, short_specs)
        },
        ...
        _ => Err(ParserError::Decoding(ParserDecodingError::BalanceNotDescribed))
    }
}
...
///
/// The function outputs the DecodedOut value in case of success.
fn deal_with_struct (v1: &[StructField], mut data: Vec<u8>, type_database: &[TypeEntry], indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
    let mut fancy_out: Vec<OutputCard> = Vec::new();
...
///
/// The function outputs the DecodedOut value in case of success.
fn deal_with_enum (v1: &[EnumVariant], mut data: Vec<u8>, type_database: &[TypeEntry], indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {
...
///
/// The function outputs the DecodedOut value in case of success.
fn decode_simple (found_ty: &str, mut data: Vec<u8>, type_database: &[TypeEntry], indent: u32, short_specs: &ShortSpecs) -> Result<DecodedOut, ParserError> {

    if data.is_empty() {return Err(ParserError::Decoding(ParserDecodingError::DataTooShort))}
                                ...
                                None => {
                                    ...
                                    match REGARRAY.captures(found_ty) {
                                        Some(caps) => {
                                            ...
                                            deal_with_array(inner_ty, number_of_elements, data, type_database, indent, short_specs)
                                        },
                                        None => {
                                            // check for special case of Balance as is or in Compact form
                                            if goto_balance(found_ty) {
                                                special_case_balance(found_ty, data, indent, short_specs)
                                            }
                                            else {
                                                ...
                                                match REGCOMPACT.captures(found_ty) {
                                                    ...
                                                    None => {
                                                        ...
                                                        if found_ty == "IdentityFields" {
                                                            special_case_identity_fields (data, type_database, indent)
                                                        }
                                                        else {
                                                            // special case of BitVec type
                                                            if found_ty == "BitVec" {special_case_bitvec(data, indent)}
                                                            else {
                                                                // special case of AccountId type
                                                                if (found_ty == "AccountId")||(found_ty == "T::AccountId") {special_case_account_id(data, indent, short_specs)}
                                                                else {
