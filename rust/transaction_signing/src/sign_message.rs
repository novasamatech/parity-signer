...
pub (crate) fn sign_as_address_key (to_sign: &Vec<u8>, multisigner: &MultiSigner, full_address: &str, pwd: Option<&str>) -> Result<SufficientCrypto, ErrorSigner> {
    match multisigner {
        MultiSigner::Ed25519(public) => {
            ...
            Ok(SufficientCrypto::Ed25519{public: public.to_owned(), signature})
        },
        MultiSigner::Sr25519(public) => {
            ...
            Ok(SufficientCrypto::Sr25519{public: public.to_owned(), signature})
        },
        MultiSigner::Ecdsa(public) => {
            ...
            Ok(SufficientCrypto::Ecdsa{public: public.to_owned(), signature})
        },
    }
}

/// Function to generate `sufficient crypto line` for given public key
fn sufficient_crypto (multisigner: &MultiSigner, address_details: &AddressDetails, to_sign: &Vec<u8>, seed_phrase: &str, pwd_entry: &str) -> Result<SufficientCrypto, ErrorSigner> {
    let pwd = {
...
pub (crate) fn sufficient_crypto_load_types (multisigner: &MultiSigner, address_details: &AddressDetails, database_name: &str, seed_phrase: &str, pwd_entry: &str) -> Result<String, ErrorSigner> {
    ...
    Ok(format!("\"sufficient\":\"{}\",\"content\":{{\"type\":\"load_types\",{}}}", sufficient, types_content.show()))
}

/// Function to generate hex line of qr data corresponding to `sufficient_crypto` for load_metadata message
pub (crate) fn sufficient_crypto_load_metadata (network_specs_key: &NetworkSpecsKey, network_version: u32, multisigner: &MultiSigner, address_details: &AddressDetails, database_name: &str, seed_phrase: &str, pwd_entry: &str) -> Result<String, ErrorSigner> {
    ...
    Ok(format!("\"sufficient\":\"{}\",\"content\":{{\"type\":\"load_metadata\",{}}}", sufficient, MetaValuesDisplay::get(&meta_values).show()))
}
