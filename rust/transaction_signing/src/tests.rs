...
// can sign a parsed transaction
    ...
    fn can_sign_transaction_1() {
        ...
        if let Action::Sign{content: reply, checksum, has_pwd: _, network_info: _} = output {
            assert!(reply == reply_known, "Error in parsing.\nReceived: {}", reply);
...
// can sign a message
    ...
    fn can_sign_message_1() {
        ...
        if let Action::Sign{content: reply, checksum, has_pwd: _, network_info} = output {
            assert!(reply == reply_known, "Error in parsing. Received:\n{}", reply);
            assert!(network_info == network_info_known, "Error in parsing. Received:\n{}", network_info);
...
Identities: 
    ...
    fn parse_transaction_alice_remarks_westend9122() {
        ...
        if let Action::Sign{content: reply, checksum: _, has_pwd: _, network_info: _} = output {
            assert!(reply == reply_known, "Error on parsing. Received:\n{}", reply);
        }
        else {panic!("Wrong action: {:?}", output)}
