...
Identities: "#;
    ...
    fn rococo_and_verifiers_1() {
        ...
        let line =
            fs::read_to_string("for_tests/add_specs_rococo-ed25519_Alice-ed25519.txt").unwrap();
        let output = produce_output(&line.trim(), dbname);
        if let Action::Stub(_, checksum, _) = output {
            handle_stub(checksum, dbname).unwrap();
        } else {
        ...
        let line =
            fs::read_to_string("for_tests/add_specs_rococo-sr25519_Alice-ed25519.txt").unwrap();
        let output = produce_output(&line.trim(), dbname);
        if let Action::Stub(_, checksum, _) = output {
            handle_stub(checksum, dbname).unwrap();
        } else {
...
Identities: "#;
    ...
    fn rococo_and_verifiers_2() {
        ...
        let line =
            fs::read_to_string("for_tests/add_specs_rococo-sr25519_Alice-sr25519.txt").unwrap();
        let output = produce_output(&line.trim(), dbname);
        if let Action::Stub(_, checksum, _) = output {
            handle_stub(checksum, dbname).unwrap();
        } else {
