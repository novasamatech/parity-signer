...
/// For each test the database is constructed in its original form from the datafiles
...
mod tests {
    ...
    use db_handling::{populate_cold, DataFiles};
    ...
    fn reload_database(dbname: &str) -> Result<(), Box<dyn std::error::Error>> {
        let metadata_contents = fs::read_to_string("for_tests/metadata_database.ts")?;
        let identities = fs::read_to_string("for_tests/identities")?;
...
// can sign a parsed transaction
    ...
    fn can_sign_transaction_1() {
        ...
        let mock_action_line = r#"{"type":"sign_transaction","checksum":"3260873712"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
...
// add_network for dock_main without verifier, then add_network with same metadata and with verifier
    ...
    fn add_network_add_two_verifiers_later() {
        ...
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"add_network","checksum":"780003847"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
...
// add_network for dock_main with verifier
    ...
    fn add_network_and_add_general_verifier() {
        ...
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"add_network_and_add_general_verifier","checksum":"1993532839"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
    ...
    fn correct_checksum_no_transaction_to_sign() {
        ...
        let mock_action_line = r#"{"type":"sign_transaction","checksum":"780003847"}"#;
    ...
    fn correct_checksum_no_approved_metadata() {
        ...
        let mock_action_line = r#"{"type":"load_metadata","checksum":"3260873712"}"#;
    ...
    fn correct_checksum_no_metadata_verifier() {
        ...
        let mock_action_line = r#"{"type":"add_metadata_verifier","checksum":"3260873712"}"#;
    ...
    fn correct_checksum_no_types_to_load() {
        ...
        let mock_action_line = r#"{"type":"load_types","checksum":"3260873712"}"#;
    ...
    fn correct_checksum_no_general_verifier() {
        ...
        let mock_action_line = r#"{"type":"add_general_verifier","checksum":"3260873712"}"#;
    ...
    fn correct_checksum_no_two_verifiers() {
        ...
        let mock_action_line = r#"{"type":"add_two_verifiers","checksum":"3260873712"}"#;
    ...
    fn correct_checksum_no_load_meta_and_upd_verifier() {
        ...
        let mock_action_line = r#"{"type":"load_metadata_and_add_general_verifier","checksum":"3260873712"}"#;
    ...
    fn correct_checksum_no_add_network() {
        ...
        let mock_action_line = r#"{"type":"add_network","checksum":"3260873712"}"#;
    ...
    fn correct_checksum_no_add_network_and_general_verifier() {
        ...
        let mock_action_line = r#"{"type":"add_network_and_add_general_verifier","checksum":"3260873712"}"#;
...
// load_metadata for westend9070 not verified, then load same metadata, but with verifier
    ...
    fn load_network_unsigned_add_verifier_later() {
        ...
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"load_metadata","checksum":"3987807029"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
        ...
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"add_metadata_verifier","checksum":"4207599612"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
...
// load_types not verified, then load same types message, but with verifier
    ...
    fn load_types_unsigned_add_verifier_later() {
        ...
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"load_types","checksum":"4127833422"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
        ...
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"add_general_verifier","checksum":"3569017006"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
...
// load_types with verifier, general verifier appears, still can load metadata without verifier, but cannot add networks unverified
    ...
    fn load_types_verified_then_test_unverified_load_metadata_and_unverified_add_network() {
        ...
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"load_types","checksum":"3138751566"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
    ...
    // loading metadata without verifier - should work        
        ...
        let reply_known = r#"{"warning":[{"index":0,"indent":0,"type":"warning","payload":"Received network metadata is not verified."}],"meta":[{"index":1,"indent":0,"type":"meta","payload":{"specname":"westend","spec_version":"9070","meta_hash":"e281fbc53168a6b87d1ea212923811f4c083e7be7d18df4b8527b9532e5f5fec"}}],"action":{"type":"load_metadata","payload":{"type":"load_metadata","checksum":"2757293651"}}}"#;
        assert!(reply == reply_known, "Error in action.\nReceived: {}", reply);
        let mock_action_line = r#"{"type":"load_metadata","checksum":"2757293651"}"#;
        let result = handle_action(&mock_action_line, PIN, PWD, DBNAME);
