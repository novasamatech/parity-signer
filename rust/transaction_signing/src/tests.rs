...
// can sign a parsed transaction
    ...
    fn can_sign_transaction_1() {
        ...
        if let Action::Sign{content, checksum, has_pwd, author_info, network_info} = output {
            ...
            let history_recorded_cut = history_recorded.replace(ALICE_SR_ALICE, r#"<alice_sr25519_//Alice>"#);
            ...
            assert!(historic_reply_cut.contains(historic_reply_known), "Received different historic reply: \n{}", historic_reply);
        }
...
Identities: 
    ...
    fn load_westend9070() {
        ...
        if let Action::Stub(reply, checksum, stub_nav) = output {
            ...
            assert!(reply_cut == reply_known, "Error in parsing. Received: \n{}", reply);
            assert!(stub_nav == stub_nav_known, "Received: \n{:?}", stub_nav);
...
Identities: 
    ...
    fn load_new_types_verified() {
        ...
        if let Action::Stub(reply, checksum, stub_nav) = output {
            ...
            let print_before = print_db_content(&dbname).replace(ALICE_SR_ALICE, r#"<alice_sr25519_//Alice>"#);
            let expected_print_before = r#"Database contents:
Metadata:
	kusama2030
	polkadot30
...
Identities: 
	...
	public_key: f606519cb8726753885cd4d0f518804a69a5e0badf36fee70feadd8044081730, encryption: sr25519, path: //polkadot, available_networks: 
		018091b171bb158e2d3848fa23a9f1c25182fb8e20313b2c1eb49219da7a70ce90c3"#;
            ...
            let print_after = print_db_content(&dbname).replace(ALICE_SR_ALICE, r#"<alice_sr25519_//Alice>"#);
            let expected_print_after = r#"Database contents:
Metadata:
	kusama2030
	polkadot30
...
Identities: 
	...
	public_key: f606519cb8726753885cd4d0f518804a69a5e0badf36fee70feadd8044081730, encryption: sr25519, path: //polkadot, available_networks: 
        ...
        if let Action::Stub(reply, checksum, stub_nav) = output {
            ...
            assert!(reply_cut == reply_known, "Error on parsing. Received: \n{}", reply);
...
Identities: 
	...
	public_key: f606519cb8726753885cd4d0f518804a69a5e0badf36fee70feadd8044081730, encryption: sr25519, path: //polkadot, available_networks: 
        ...
        if let Action::Sign{content, checksum, has_pwd, author_info, network_info} = output {
            ...
            let author_info_cut = author_info
                .replace(ALICE_SR_ROOT, r#"<alice_sr25519_root>"#);
            assert!(content_cut == content_known, "Received: \n{}", content);
            assert!(author_info_cut == author_info_known, "Received: \n{}", author_info);
            assert!(network_info == network_info_known, "Received: \n{}", network_info);
...
Identities: 
	...
	public_key: f606519cb8726753885cd4d0f518804a69a5e0badf36fee70feadd8044081730, encryption: sr25519, path: //polkadot, available_networks: 
        ...
        if let Action::Read(reply) = output {assert!(reply==reply_known, "Received: \n{}", reply);}
        else {panic!("Wrong action: {:?}", output)}
